...
export default function VagalHumming() {
  ...
  const circleScale =
    phase === "inhale" ? 0.6 + 0.4 * (1 - phaseTime / INHALE_SEC) : 1;
        ...
        <ToolBody>
          {!isRunning ? (
            <>
              <ToolInfo>
                <p>
                  Hum on the exhale as long as comfortable, then begin the next
                  cycle when you are ready.
                </p>
              ...
              <div>
                <p className="text-sm font-medium mb-2">Sound</p>
                <div className="flex gap-2">
                  {SOUNDS.map((s, i) => (
                    <Button
                      key={s.label}
                      variant={soundIndex === i ? "default" : "outline"}
                      size="sm"
            ...
            <>
              ...
              <p className="text-center text-sm">{sound.guidance}</p>
