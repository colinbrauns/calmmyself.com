...
export default function OceanBreath() {
  ...
  if (completed) {
    return (
      ...
      >
        <ShareInline
          title="Ocean Breath"
          text="Ujjayi ocean breathing on CalmMyself"
        />
      </CompletionCard>
    );
  }
  ...
  return (
    <ToolCard>
        ...
        {breath.status === "idle" && timeLeft === 0 ? (
          ...
          <>
            <div
              ...
              className="relative h-32 rounded-lg overflow-hidden bg-gradient-to-b from-teal-50 to-cyan-100 dark:from-teal-950 dark:to-cyan-950"
            >
              <motion.div
                className="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-teal-400/40 to-transparent"
                animate={{ y: waveY }}
                transition={{
                  duration: phase === "inhale" ? INHALE : EXHALE,
                  ease: "easeInOut",
                }}
              />
              <svg
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <p className="text-2xl font-bold">{breath.current.label}</p>
                <p className="text-lg">{breath.displaySeconds}s</p>
              </div>
            </div>
      ...
      <ToolFooter>
        <EvidenceNote>
          Ujjayi breathing uses a slight throat constriction to create a soft
          ocean-like sound. Breathe through the nose without strain.
        </EvidenceNote>
        <ShareInline
          title="Ocean Breath"
          text="Ujjayi ocean breathing on CalmMyself"
