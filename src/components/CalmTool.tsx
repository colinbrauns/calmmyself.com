...
const toneClasses = {
  calm: {
    ...
    phase: "",
    progress: "bg-sky-500",
    info: "bg-sky-50/70 border-sky-100 dark:bg-sky-950/20 dark:border-sky-900/40",
    button: "calm",
  },
  grounding: {
    icon: "",
    ...
    info: "bg-amber-50/70 border-amber-100 dark:bg-amber-950/20 dark:border-amber-900/40",
    button: "grounding",
  },
  teal: {
    icon: "",
    ...
    info: "bg-teal-50/70 border-teal-100 dark:bg-teal-950/20 dark:border-teal-900/40",
    button: "calm",
  },
  amber: {
    icon: "",
  ...
  neutral: {
    ...
    phase: "",
    progress: "bg-sky-500",
    info: "bg-gray-50/80 border-gray-200 dark:bg-gray-800/50 dark:border-gray-700",
    button: "calm",
  },
} as const;
...
export function ToolCard({
  ...
  return (
    <Card
      className={`mx-auto max-w-md overflow-hidden rounded-lg ${className}`}
    >
      {children}
    </Card>
  );
...
export function ToolBody({
  ...
  return (
    <CardContent className={`space-y-5 px-5 pb-5 ${className}`}>
      {children}
    </CardContent>
  );
}
...
export function PhaseReadout({
  ...
  return (
    <div data-testid="phase-readout" className="min-h-[64px] text-center">
      <div
        className={`mb-1 min-h-[32px] text-2xl font-semibold leading-tight ${classes.phase}`}
      >
...
export function ToolInfo({
  ...
  return (
    <div
      className={`rounded-lg border p-3 text-sm leading-relaxed ${getTone(tone).info} ${className}`}
    >
      {children}
    </div>
  );
...
export function EvidenceNote({ children }: { children: ReactNode }) {
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50/80 p-3 text-xs leading-relaxed dark:border-gray-700 dark:bg-gray-800/50">
      {children}
    </div>
  );
}
  ...
  return (
    <ToolCard>
      <ToolBody className="py-12 text-center">
        ...
        <Button
          onClick={onAction}
          variant={getTone(tone).button as ButtonProps["variant"]}
          size="lg"
        >
          {actionLabel}
        </Button>
...
export function DurationSelector({
  ...
  const selectedClass =
    tone === "teal" ? "bg-teal-600 text-white" : "bg-sky-600 text-white";
    ...
    <div className="flex justify-center gap-2">
      {values.map((option) => (
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${option === value ? selectedClass : "bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700"}`}
        >
          {option} {unit}
        </button>
      ))}
