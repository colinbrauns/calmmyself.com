...
export default function AffirmationsMantras() {
  ...
  const currentNum =
    CATEGORIES.slice(0, categoryIndex).reduce((a, c) => a + c.items.length, 0) +
    lineIndex +
    1;
  const isLast =
    categoryIndex === CATEGORIES.length - 1 &&
    lineIndex === category.items.length - 1;
  ...
  const prev = () => {
    if (lineIndex > 0) setLineIndex(lineIndex - 1);
      ...
      setLineIndex(CATEGORIES[prevCat].items.length - 1);
    }
  };
  const reset = () => {
    setCategoryIndex(0);
        ...
        <CardContent className="text-center py-12 space-y-5">
          ...
          >
            ...
            <p className="mt-2 text-sm">
              You can come back to any of these phrases whenever you need a
              gentler inner voice. They don&apos;t need to be perfect \u2014
              just present.
            </p>
          </motion.div>
  ...
  return (
    <Card className="max-w-md mx-auto">
      <CardHeader className="text-center">
        ...
        <CardDescription>
          {category.name} \u2022 {currentNum} of {total}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="flex gap-2">
          <Button
            ...
            size="lg"
            className="flex-1"
            disabled={categoryIndex === 0 && lineIndex === 0}
          >
            Previous
          </Button>
          <Button
          ...
          <Button onClick={next} variant="calm" size="lg" className="flex-1">
            {isLast ? "Finish" : "Next"}
          </Button>
        </div>
      ...
      <div className="px-6 pb-6 pt-0">
        <div className="pt-4 mt-2 border-t border-gray-100 dark:border-gray-800 space-y-3">
          <div className="text-xs bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 p-3 rounded-xl">
            About: Brief self-statements can guide attention and foster a kinder
            inner tone.
            <br />
            Evidence: Self-compassion and attentional reappraisal are associated
            with reduced stress reactivity.
          </div>
          <ShareInline
            title="Affirmations & Mantras"
            text="Use supportive affirmations on CalmMyself"
          />
        </div>
      </div>
    </Card>
