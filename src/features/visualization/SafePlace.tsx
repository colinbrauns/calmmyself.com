...
function drawAurora(
  ...
  for (let band = 0; band < 3; band++) {
    ...
    for (let x = 0; x <= w; x += 10) {
      const y =
        yBase +
        Math.sin(x * 0.005 + time * 0.3 + band * 1.5) * 30 +
...
function drawCentralGlow(
  ...
  const radius =
    (w * 0.3 + Math.sin(time * 0.5) * 20) * intensity * breathScale;
  const cx = w / 2;
  const cy = h * 0.45;
...
function drawBranches(
  ...
  function drawBranch(
    ...
    drawBranch(
      endX,
      endY,
      angle - 0.45 + Math.sin(time * 0.3) * 0.08,
      length * 0.68,
      depth - 1,
    );
    drawBranch(
      endX,
      endY,
      angle + 0.45 + Math.cos(time * 0.3) * 0.08,
      length * 0.68,
      depth - 1,
    );
  }
...
function SanctuaryCanvas({
  ...
  const draw = useCallback(() => {
    ...
    const targetCount =
      phase === 0 ? 0 : phase === 1 ? 25 : phase === 2 ? 40 : 50;
  ...
  const prevQuestion = () => {
    if (currentQuestionIndex > 0) {
      ...
      setCurrentQuestionIndex(
        SAFE_PLACE_PROMPTS[prevCategoryIndex].questions.length - 1,
      );
    }
  };
  ...
  const summaryKeywords = useMemo(() => {
    const allWords = responses
      ...
      .filter(
        (word) =>
          word.length > 4 &&
          !["place", "there", "their", "about"].includes(word),
      );
    const unique: string[] = [];
          ...
          <Card className="mx-auto max-w-xl bg-white/20 backdrop-blur-xl border-white/20 shadow-2xl">
            <CardHeader className="text-center">
              ...
              <CardDescription className="text-white/80">
                A snapshot of the sanctuary you created — keep it close for when
                you need calm.
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-6">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {summaryKeywords.length ? (
                    ...
                    <p className="text-sm text-white/70">
                      Spend more time with the prompts to build a richer
                      description.
                    </p>
                  )}
                </div>
              ...
              <div className="space-y-4 max-h-60 overflow-y-auto pr-1">
                {SAFE_PLACE_PROMPTS.map((group) => (
                  <div
                    key={group.category}
                    className="rounded-lg bg-white/10 backdrop-blur-sm p-4 border border-white/10"
                  >
                    ...
                    <div className="space-y-2 text-sm text-white/90">
                      {group.questions.map((question) => {
                        const index = flatPrompts.findIndex(
                          (prompt) =>
                            prompt.category === group.category &&
            ...
            <CardContent className="space-y-6">
              <div className="bg-white/10 backdrop-blur-sm p-6 rounded-lg text-center border border-white/10">
                ...
                <p className="text-white/90 leading-relaxed mb-4">
                  This guided visualization will help you create a detailed
                  mental image of a safe, peaceful place. Watch your sanctuary
                  come to life as you answer each question.
                </p>
              </div>
      ...
      <div className="relative z-10 flex items-center justify-center min-h-[600px] p-4">
        <Card className="max-w-md w-full bg-white/15 backdrop-blur-xl border-white/20 shadow-2xl">
          <CardHeader className="text-center pb-3">
            <CardTitle className="text-white drop-shadow-lg text-lg">
              Safe Place Visualization
            </CardTitle>
          ...
          <CardContent className="space-y-5">
            ...
            {/* Category */}
            <div className="text-center">
              <div className="inline-block px-3 py-1 bg-white/15  rounded-full text-sm font-medium border border-white/10">
            ...
            <div className="bg-white/8 backdrop-blur-sm border border-white/10 rounded-lg p-3">
              <p className="text-sm text-white/70">
                💡 <strong className="text-white/90">Tip:</strong> Take your
                time. Close your eyes and really experience this place with all
                your senses before moving on.
              </p>
            </div>
