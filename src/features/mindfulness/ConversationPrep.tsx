...
import { MessageCircle, CheckCircle2, RotateCcw, ArrowLeft, ArrowRight } from 'lucide-react'
...
const STEPS: PrepStep[] = [
  {
    ...
    description: 'For example: honesty, kindness, clarity, staying present, listening.',
    placeholder: 'e.g. Being honest but kind...',
    inputType: 'input',
  },
  {
    label: 'What is in your control?',
    description: 'Your tone, what you say (or don\'t), how long you stay, whether you take breaks.',
      ...
      <Card className="max-w-md mx-auto">
        <CardContent className="text-center py-10 space-y-5">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-100">You&apos;re ready \ud83d\udcaa</h3>
            <p className="text-gray-600 dark:text-gray-400 mt-2 text-sm">
              You&apos;ve oriented yourself. Take a breath and carry your phrase with you.
            </p>
          </motion.div>
          {answers[2] && (
            <motion.div
            ...
            >
              <p className="text-xs text-green-600 dark:text-green-400 font-bold uppercase mb-1">Your phrase</p>
              <p className="text-sm text-green-800 dark:text-green-200 italic">&ldquo;{answers[2]}&rdquo;</p>
            </motion.div>
          )}
    ...
    <Card className="max-w-md mx-auto">
      <CardHeader className="text-center">
        <div className="flex items-center justify-center gap-2 mb-2">
          <MessageCircle className="w-5 h-5 text-calm-600" />
      ...
      <CardContent className="space-y-6">
        ...
        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
          <div className="bg-green-400 h-2 rounded-full transition-all duration-300" style={{ width: `${((stepIndex + 1) / STEPS.length) * 100}%` }} />
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {step.inputType === 'input' ? (
              <input
                type="text"
                className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-green-500 focus:border-green-500"
      ...
      <div className="px-6 pb-6 pt-0"><div className="pt-4 mt-2 border-t border-gray-100 dark:border-gray-800">
        <ShareInline title="Before\u2011Conversation Prep" text="Prep for hard conversations with a short values-based exercise on CalmMyself." />
      </div></div>
    </Card>
  )
}
