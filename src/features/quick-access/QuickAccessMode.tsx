...
const QUICK_TECHNIQUES: QuickTechnique[] = [
  {
    ...
    title: "Double Inhale",
    instruction:
      "Take two quick inhales through your nose, then one long exhale through your mouth",
    duration: 6000,
    type: "breathing",
  },
  {
    ...
    title: "Feet on Floor",
    instruction:
      "Press your feet firmly into the ground. Feel the connection and stability",
    duration: 8000,
    type: "grounding",
  },
  {
    ...
    title: "Shoulder Drop",
    instruction:
      "Lift your shoulders up to your ears, hold for 3 seconds, then let them drop",
    duration: 8000,
    type: "physical",
  },
  {
    ...
    instruction:
      "Imagine splashing cold water on your face, or actually do it if possible",
    duration: 5000,
    type: "physical",
  },
  {
  ...
  },
  {
    ...
    title: "Hand Squeeze",
    instruction:
      "Make tight fists, squeeze for 5 seconds, then release and shake your hands",
    duration: 10000,
    type: "physical",
  },
  {
...
export default function QuickAccessMode() {
        ...
        setTimeRemaining((time) => {
          if (time <= 100) {
            ...
            return QUICK_TECHNIQUES[
              Math.min(currentTechnique + 1, QUICK_TECHNIQUES.length - 1)
            ].duration;
          }
          return time - 100;
        });
    ...
    return (
      <Card className="max-w-md mx-auto">
        <CardContent className="text-center py-8">
          ...
          <p className="mb-4">
            You completed {completedTechniques.length} technique
            {completedTechniques.length !== 1 ? "s" : ""} in {totalTime}{" "}
            seconds.
          </p>

          <div className="bg-green-50 dark:bg-green-950/30 rounded-xl p-4 mb-6">
            <p className="text-sm">
              <strong>Great job!</strong> You&rsquo;ve given yourself some quick
              relief. If you need more support, try a longer technique from the
              main menu.
            </p>
          </div>
      ...
      <Card className="max-w-md mx-auto">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            {["🫁 Breathing", "🌱 Grounding", "💪 Physical"].map(
              (type, index) => (
  ...
  const progress =
    ((currentTech.duration - timeRemaining) / currentTech.duration) * 100;
  ...
  return (
    <Card className="max-w-md mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        {/* Current Technique */}
        <div
          className={`rounded-xl p-6 text-center ${typeColor === "blue" ? "bg-blue-100" : typeColor === "green" ? "bg-green-100" : typeColor === "purple" ? "bg-purple-100" : "bg-gray-100"}`}
        >
          <div className="text-3xl mb-3">{getTypeIcon(currentTech.type)}</div>

          <h3
            className={`text-xl font-semibold mb-3 ${typeColor === "blue" ? "" : typeColor === "green" ? "" : typeColor === "purple" ? "" : ""}`}
          >
            {currentTech.title}
          </h3>

          <p
            className={`leading-relaxed mb-4 ${typeColor === "blue" ? "" : typeColor === "green" ? "" : typeColor === "purple" ? "" : ""}`}
          >
            {currentTech.instruction}
          </p>

          <div
            className={`text-2xl font-bold ${typeColor === "blue" ? "" : typeColor === "green" ? "" : typeColor === "purple" ? "" : ""}`}
          >
        ...
        <div
          className={`w-full rounded-full h-2 ${typeColor === "blue" ? "bg-blue-100" : typeColor === "green" ? "bg-green-100" : typeColor === "purple" ? "bg-purple-100" : "bg-gray-100"}`}
        >
          <div
            className={`h-2 rounded-full transition-all duration-100 ease-linear ${typeColor === "blue" ? "bg-blue-500" : typeColor === "green" ? "bg-green-500" : typeColor === "purple" ? "bg-purple-500" : "bg-gray-500"}`}
        ...
        <div className="flex justify-center space-x-2">
          {QUICK_TECHNIQUES.map((_, index) => (
            <div
              key={index}
              className={`w-2 h-2 rounded-full ${completedTechniques.includes(QUICK_TECHNIQUES[index].id) ? "bg-green-500" : index === currentTechnique ? "bg-yellow-500" : "bg-gray-300"}`}
            />
          ))}
        </div>
      </CardContent>
