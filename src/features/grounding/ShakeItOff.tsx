...
const PHASES = [
  ...
  { text: 'Stand still. Notice how your body feels.', duration: 20, intensity: 0 },
]
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      setPhaseTime(prev => {
        if (prev <= 1) {
          setPhaseIndex(pi => {
            if (pi >= PHASES.length - 1) { stop(); return pi }
            const next = pi + 1
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Somatic stress release through guided shaking and movement</CardDescription>
        </CardHeader>
          ...
          {!isRunning ? (
            <>
              <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-4 text-sm text-gray-600 dark:text-gray-300 space-y-2">
                <p>Animals naturally shake after a stressful encounter to release tension. This guided sequence progressively builds shaking through your body, then returns to stillness.</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Duration: ~2 minutes</p>
              </div>
            ...
            <>
              ...
              <div className="relative h-48 flex items-center justify-center overflow-hidden rounded-xl bg-gray-50 dark:bg-gray-800/50">
                {Array.from({ length: 12 }).map((_, i) => (
                  <motion.div
                    ...
                    animate={intensity > 0 ? {
                      x: [0, (Math.random() - 0.5) * 40 * intensity, 0, (Math.random() - 0.5) * 40 * intensity, 0],
                      y: [0, (Math.random() - 0.5) * 40 * intensity, 0, (Math.random() - 0.5) * 40 * intensity, 0],
                    } : { x: 0, y: 0 }}
                ...
                <div className="absolute bottom-3 left-3 right-3">
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <motion.div
                      ...
                      animate={{
                        width: `${intensity * 100}%`,
                        backgroundColor: intensity > 0.7 ? '#f59e0b' : intensity > 0 ? '#fbbf24' : '#d1d5db',
                      }}
              ...
              <p className="text-xs text-center text-gray-400 dark:text-gray-500">Step {phaseIndex + 1} of {PHASES.length}</p>
