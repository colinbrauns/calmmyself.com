...
export default function NoiseAndAmbience() {
  ...
  const lfoNodesRef = useRef<{ osc?: OscillatorNode; gain?: GainNode } | null>(
    null,
  );
  ...
  const buildNoise = (ctx: AudioContext, type: NoiseType) => {
    ...
    src.connect(f1);
    f1.connect(f2);
    f2.connect(mainGainRef.current!);
    src.start();
    srcRef.current = src;
  };
        ...
        <CardContent className="text-center py-10 space-y-5">
          ...
          >
            ...
            <p className="mt-2 text-sm">
              You listened for {formatTime(duration)}. Take a moment to notice
              how the quiet feels now.
            </p>
          </motion.div>
  ...
  return (
    <Card className="max-w-md mx-auto">
      <CardHeader className="text-center">
        ...
        <CardDescription>
          White/pink/brown noise and nature soundscapes
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="flex gap-2 justify-center">
          <Button
            variant={mode === "noise" ? "grounding" : "outline"}
            onClick={() => setMode("noise")}
          >
            Noise
          </Button>
          <Button
            variant={mode === "ambience" ? "grounding" : "outline"}
            onClick={() => setMode("ambience")}
          >
            Ambience
          </Button>
        </div>
        ...
        {mode === "noise" ? (
          <div className="grid grid-cols-2 gap-2">
            {(["white", "pink", "brown", "blue", "violet"] as NoiseType[]).map(
              (t) => (
                <Button
                  key={t}
                  variant={noise === t ? "grounding" : "outline"}
                  onClick={() => {
                    setNoise(t);
                    if (isPlaying) start();
                  }}
          ...
          <div className="grid grid-cols-3 gap-2">
            <Button
              variant={ambience === "ocean" ? "grounding" : "outline"}
              onClick={() => {
                setAmbience("ocean");
                if (isPlaying) start();
              }}
            ...
            <Button
              variant={ambience === "rain" ? "grounding" : "outline"}
              onClick={() => {
                setAmbience("rain");
                if (isPlaying) start();
              }}
            ...
            <Button
              variant={ambience === "rainforest" ? "grounding" : "outline"}
              onClick={() => {
                setAmbience("rainforest");
                if (isPlaying) start();
              }}
              className="flex items-center gap-2"
            >
      ...
      <div className="px-6 pb-6 pt-0">
        <div className="pt-4 mt-2 border-t border-gray-100 dark:border-gray-800">
          <ShareInline
            ...
            text="Play noise and nature ambience on CalmMyself"
          />
        </div>
      </div>
    </Card>
