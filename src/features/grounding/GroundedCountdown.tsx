...
export default function GroundedCountdown() {
    ...
    return (
      <Card className="max-w-md mx-auto">
        <CardContent className="text-center py-12 space-y-5">
          ...
          >
            <h3 className="text-xl font-semibold">Grounded 🌿</h3>
            <p className="mt-2 text-sm">
              Three rounds complete. Notice how your feet feel against the floor
              right now — a little more connected, maybe a little steadier.
            </p>
          </motion.div>
  ...
  const overallProgress =
    ((round - 1) * COUNTS.length + index + 1) / (3 * COUNTS.length);
  ...
  return (
    <Card className="max-w-md mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="text-center space-y-3">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-amber-100 dark:bg-amber-900/30 text-4xl font-bold"
            >
              {current}
            </motion.div>
          </AnimatePresence>
        ...
        <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-3 text-xs flex items-start gap-2">
          <ChevronDown className="w-4 h-4 mt-0.5 flex-shrink-0" />
          <p>
            You can do this while walking, pacing, or seated by pressing your
