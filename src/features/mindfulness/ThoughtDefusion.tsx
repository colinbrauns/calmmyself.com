        ...
        <CardContent className="py-10 space-y-6">
          <div className="text-center">
            ...
            <h3 className="text-xl font-semibold mb-2">
              Notice the distance 🌬️
            </h3>
            <p className="text-sm mb-6">
              See how each layer creates more space between you and the thought?
            </p>
          </div>
          <motion.div
            initial={{ opacity: 0 }}
          ...
          <ShareInline
            ...
            text="ACT thought defusion technique on CalmMyself"
          />
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card className="max-w-md mx-auto">
      <CardHeader className="text-center">
        ...
        <CardDescription>
          Create distance from distressing thoughts using an ACT technique
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="flex justify-center gap-2">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className={`w-10 h-1.5 rounded-full transition-colors ${i <= step ? "bg-violet-500" : "bg-gray-200 dark:bg-gray-700"}`}
            />
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          {step === 0 && (
            <motion.div
              ...
              className="space-y-4"
            >
              <p className="text-center">What thought is bothering you?</p>
              <textarea
                value={thought}
                onChange={(e) => setThought(e.target.value)}
                placeholder="e.g., I'm not good enough"
                rows={3}
                className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 focus:outline-none focus:ring-2 focus:ring-violet-300 dark:focus:ring-violet-700 text-sm resize-none"
              />{" "}
          ...
          {step === 1 && (
            ...
            >
              ...
              <p className="text-center text-sm">
                Now read this version out loud:
              </p>{" "}
          ...
          {step === 2 && (
            <motion.div
              ...
              className="space-y-4"
            >
              <p className="text-center text-sm">And now this one:</p>
              <div className="bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-800/50 rounded-xl p-4">
                <p className="text-lg">&ldquo;{prefixed2}&rdquo;</p>
              </div>
              <p className="text-center text-xs">
                Even more space. The thought is still there, but you&apos;re
                observing it from further away.
              </p>
              <div className="flex justify-center">
                <Button onClick={() => setCompleted(true)} variant="calm">
