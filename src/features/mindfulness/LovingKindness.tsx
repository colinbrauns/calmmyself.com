...
const PHASES = [
  { name: "Self", start: 0, end: 4, description: "Sending love to yourself" },
  {
    ...
    description: "Sending love to someone dear",
  },
  {
    name: "All Beings",
    start: 10,
    end: 14,
    description: "Sending love to all beings",
  },
];
...
export default function LovingKindness() {
  ...
  const [timeRemaining, setTimeRemaining] = useState(
    LOVING_KINDNESS_PHRASES[0].duration,
  );
  ...
  const currentPhase = PHASES.find(
    (phase) => currentPhrase >= phase.start && currentPhrase <= phase.end,
  );
    ...
    if (isActive && !isComplete) {
      interval = setInterval(() => {
        setTimeRemaining((time) => {
          if (time <= 100) {
            nextPhrase();
            return LOVING_KINDNESS_PHRASES[
              Math.min(currentPhrase + 1, LOVING_KINDNESS_PHRASES.length - 1)
            ].duration;
          }
          return time - 100;
        });
  ...
  const progress =
    ((currentPhraseData.duration - timeRemaining) /
      currentPhraseData.duration) *
    100;
  const overallProgress =
    ((currentPhrase + 1) / LOVING_KINDNESS_PHRASES.length) * 100;
    ...
    return (
      <Card className="max-w-md mx-auto">
        <CardContent className="text-center py-8">
          ...
          <p className="mb-6">
            You&rsquo;ve sent loving kindness to yourself, a loved one, and all
            beings. Notice how your heart feels now - perhaps softer, warmer,
            more open.
          </p>
    ...
    <Card className="max-w-md mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-between text-sm">
          <span>
            {currentPhase?.name} ({currentPhrase + 1}/
            {LOVING_KINDNESS_PHRASES.length})
          </span>
        ...
        {currentPhase && (
          <div className="text-center">
            ...
            <div
              className={`inline-block px-3 py-1 rounded-full bg-gradient-to-r ${getPhaseColor(currentPhase.name)} text-sm font-medium`}
            >
              {currentPhase.name}
            </div>
          </div>
        ...
        <div className="bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-950/30 dark:to-purple-950/30 rounded-xl p-8 text-center">
          <div className="mb-4">
            <Heart className="w-12 h-12 mx-auto mb-3" fill="currentColor" />
          </div>
        ...
        {/* Instructions */}
        <div className="text-sm text-center bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl">
        ...
        <div className="grid grid-cols-3 gap-2">
          {PHASES.map((phase, index) => (
            <div key={index} className="text-center">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${currentPhase?.name === phase.name ? "bg-pink-500 text-white" : currentPhrase > phase.end ? "bg-green-500 text-white" : "bg-gray-200"}`}
              >
                {index + 1}
              </div>
              <p className="text-xs mt-1">{phase.name}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center space-x-3">
          <Button
            ...
            aria-label={
              isActive
                ? "Pause loving kindness practice"
                : "Start loving kindness practice"
            }
          >
