...
export default function PresentMoment() {
  ...
  const getRandomPrompt = () => {
    const availablePrompts = PRESENT_MOMENT_PROMPTS.filter(
      (_, index) => index !== currentPrompt,
    );
    const randomIndex = Math.floor(Math.random() * availablePrompts.length);
    const newPromptIndex = PRESENT_MOMENT_PROMPTS.findIndex(
      (prompt) => prompt === availablePrompts[randomIndex],
    );
    ...
    return (
      <Card className="max-w-md mx-auto">
        <CardContent className="text-center py-10 space-y-5">
          ...
          >
            <h3 className="text-xl font-semibold">Beautifully present ✨</h3>
            <p className="mt-2 text-sm">
              You spent {formatTime(duration)} in present moment awareness. This
              moment is always available to you — no app needed.
            </p>
          </motion.div>
          ...
          <ShareInline
            ...
            text="Practice present moment awareness on CalmMyself"
          />
        </CardContent>
      </Card>
    );
      ...
      <Card className="max-w-md mx-auto">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl">
            <p className="text-sm mb-2 font-medium">How to practice:</p>
    ...
    <Card className="max-w-md mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <textarea
            className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 resize-none"
            rows={3}
