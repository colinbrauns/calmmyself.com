...
export function useBreathPattern<TPhase extends string>({
    ...
    (now: number) => {
      ...
      while (elapsedMs >= currentPattern[phaseIndex].durationMs) {
        ...
        if (phaseIndex === 0) {
          ...
          if (
            maxCyclesRef.current !== undefined &&
            cycleCount >= maxCyclesRef.current
          ) {
      ...
      const shouldPaint =
        phaseChanged || now - lastPaintAtRef.current >= tickMs;
  ...
  const phaseProgress = Math.min(
    1,
    Math.max(0, snapshot.elapsedMs / phaseDurationMs),
  );
