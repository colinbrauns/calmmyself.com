...
function generatePuffs(
  ...
  return [
    { dx: v(), dy: v(), r: (28 + Math.random() * 6) * scale },
    {
      dx: (-24 + v()) * scale,
      dy: 4 + v(),
      r: (20 + Math.random() * 5) * scale,
    },
    {
      dx: (24 + v()) * scale,
      dy: 4 + v(),
      r: (23 + Math.random() * 5) * scale,
    },
    {
      dx: (-14 + v()) * scale,
      dy: -11 + v(),
      r: (18 + Math.random() * 5) * scale,
    },
    {
      dx: (14 + v()) * scale,
      dy: -9 + v(),
  ...
  layers.forEach((layer, li) => {
    for (let i = 0; i < layer.count; i++) {
      ...
      clouds.push({
        ...
        y:
          (layer.yRange[0] +
            Math.random() * (layer.yRange[1] - layer.yRange[0])) *
          canvasH,
        scale,
        speed:
          layer.speedRange[0] +
...
function getSkyColors(t: number): {
  ...
  const nightAmount =
    t > 0.6 && t < 0.9
      ? Math.min(1, (t - 0.6) / 0.15)
      : t >= 0.9
        ? Math.max(0, 1 - (t - 0.9) / 0.1)
        : 0;
  ...
  return {
    ...
    sunColor:
      nightAmount > 0.5 ? "rgba(220,220,255,0.8)" : "rgba(255,240,200,1)",
    sunY,
...
export default function CloudVisualization() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (sky.nightAmount > 0) {
        ...
        starsRef.current.forEach((star) => {
          const twinkle =
            ...
            0.7 *
              (0.5 +
                0.5 * Math.sin(timeS * star.twinkleSpeed + star.twinkleOffset));
      ...
      if (sky.sunY < 0.85) {
        ...
        if (sky.nightAmount < 0.3 && sky.sunY < 0.6) {
          ...
          for (let i = 0; i < 6; i++) {
            ...
            ctx.lineTo(
              sunScreenX + Math.cos(angle - 0.04) * len,
              sunScreenY + Math.sin(angle - 0.04) * len,
            );
            ctx.lineTo(
              sunScreenX + Math.cos(angle + 0.04) * len,
              sunScreenY + Math.sin(angle + 0.04) * len,
      ...
      currentThoughts.forEach((tc) => {
        ...
        const fadeOut =
          tc.x > w - 150 ? Math.max(0, 1 - (tc.x - (w - 150)) / 150) : 1;
    ...
    return (
      <div className="max-w-md mx-auto">
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg p-8 text-center space-y-5">
          ...
          >
            <h3 className="text-xl font-semibold">Beautiful session ☁️</h3>
            <p className="mt-2 text-sm">
              You spent {formatTime(duration)} watching thoughts drift by like
              clouds. Remember — you are the sky, not the clouds.
            </p>
          </motion.div>
          ...
          <ShareInline
            ...
            text="Use a cloud visualization for racing thoughts on CalmMyself."
          />
        </div>
      </div>
    );
    ...
    return (
      <div className="max-w-md mx-auto">
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg p-8 text-center space-y-6"
        >
          <div className="flex items-center justify-center gap-2">
            <Cloud className="w-6 h-6" />
            <h2 className="text-xl font-semibold">Thoughts as Clouds</h2>
          </div>
          <p className="text-sm">
            Let thoughts drift by instead of trying to stop them. Type a thought
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="text-xs bg-calm-50 dark:bg-gray-800/50 border border-calm-100 dark:border-gray-700 p-3 rounded-md text-center">
        You don&apos;t need to push the thoughts away. Just place them in the
        sky and let the wind carry them at its own pace.
      </div>
    </div>
  );
}
