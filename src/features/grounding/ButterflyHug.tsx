...
export default function ButterflyHug() {
  ...
  useEffect(() => {
    ...
    return () => { if (tapRef.current) clearInterval(tapRef.current) }
  }, [isRunning, tempo.bps])
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      <Card>
        <CardHeader>
          ...
          <CardDescription>WHO-endorsed bilateral stimulation for calm and safety</CardDescription>
        </CardHeader>
          ...
          {!isRunning ? (
            <>
              <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-4 text-sm text-gray-600 dark:text-gray-300 space-y-2">
                <p className="font-medium text-gray-800 dark:text-gray-100">How to do it:</p>
                <ol className="list-decimal list-inside space-y-1">
                  <li>Cross your arms over your chest</li>
                  <li>Place each hand on the opposite shoulder</li>
                  <li>Alternate tapping left and right in rhythm with the visual</li>
                  <li>Think of a safe, calm place or just notice sensations</li>
                </ol>
              </div>

              <div>
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Tempo</p>
                <div className="flex gap-2">
                  {TEMPOS.map((t, i) => (
                    <Button key={t.label} variant={tempoIndex === i ? 'default' : 'outline'} size="sm" onClick={() => setTempoIndex(i)}>
                      {t.label} ({t.bps}/s)
                    </Button>
                  ))}
                </div>
            ...
            <>
              ...
              <div className="relative h-48 flex items-center justify-center">
                <svg viewBox="0 0 200 160" className="h-full w-auto">
                  {/* Shoulders */}
                  <ellipse cx="60" cy="80" rx="40" ry="50" fill="none" stroke="#d1d5db" strokeWidth="1.5" className="dark:stroke-gray-600" />
                  <ellipse cx="140" cy="80" rx="40" ry="50" fill="none" stroke="#d1d5db" strokeWidth="1.5" className="dark:stroke-gray-600" />
                  {/* Left hand on right shoulder */}
                  <motion.circle cx="140" cy="50" r="14"
                    animate={{ opacity: side === 'left' ? 1 : 0.2, scale: side === 'left' ? 1.15 : 1 }}
                    transition={{ duration: 0.15 }}
                    fill="#6ee7b7" className="dark:fill-emerald-400"
                  />
                  {/* Right hand on left shoulder */}
                  <motion.circle cx="60" cy="50" r="14"
                    animate={{ opacity: side === 'right' ? 1 : 0.2, scale: side === 'right' ? 1.15 : 1 }}
                    transition={{ duration: 0.15 }}
                    fill="#6ee7b7" className="dark:fill-emerald-400"
                  />
                  <text x="100" y="145" textAnchor="middle" className="fill-gray-500 dark:fill-gray-400 text-xs" fontSize="12">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{tempo.label} tempo</p>
              </div>
