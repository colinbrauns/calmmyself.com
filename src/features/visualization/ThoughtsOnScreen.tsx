  ...
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D, w: number, h: number, time: number) => {
      ...
      beamGrad.addColorStop(
        0,
        `rgba(255, 240, 200, ${beamBrightness * flicker})`,
      );
      ...
      if (textPhaseRef.current === "idle") {
        ...
        const charsToShow = Math.floor(
          textProgressRef.current * fullText.length,
        );
      ...
      if (grainDataRef.current) {
        try {
          ctx.putImageData(grainDataRef.current, screenX, screenY);
        } catch {
          /* skip */
        }
      }
      ...
      for (const mote of dustMotesRef.current) {
        ...
        const beamLeft =
          screenX + (mote.y / (h * 0.7)) * (w * 0.92 - screenX - screenW) * -1;
        const beamRight =
          screenX +
          screenW +
          (1 - mote.y / (h * 0.7)) * (w * 0.92 - screenX - screenW);
      ...
      for (let x = screenX; x < screenX + screenW; x += 2) {
        const val =
          Math.sin((x - screenX) * 0.02 + time * 1.5) * 3 +
          ...
          <CardContent className="text-center py-10 space-y-5">
            ...
            >
              ...
              <p className="mt-2 text-sm font-mono">
                You watched your thoughts for {formatTime(duration)}. You are
          ...
          <Card className="bg-[#0a0a0a] border-gray-900 shadow-2xl">
            <CardHeader className="text-center">
              <div className="flex items-center justify-center gap-2 mb-1">
                ...
                <CardTitle className="text-lg">Thoughts on a Screen</CardTitle>
              </div>
        ...
        <CardContent className="space-y-4 px-3 sm:px-6">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 p-3 bg-[#111] border border-gray-800 rounded-lg placeholder-gray-600 font-mono text-sm focus:ring-1 focus:ring-amber-900/50 focus:border-amber-900/50 outline-none"
            ...
            <Button
              ...
              disabled={!input.trim()}
              variant="calm"
              className="bg-gray-800 hover:bg-gray-700 border-gray-700 font-mono"
            >
              Project
            </Button>
          </div>

          <div className="text-xs bg-[#0f0f0f] border border-gray-800/50 p-3 rounded-lg font-mono">
