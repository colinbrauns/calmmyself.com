...
class RecommendationEngine {
  ...
  static updateStats() {
    ...
    for (const entry of usage) {
      ...
      existing.averageDuration =
        (existing.averageDuration + entry.duration) / existing.totalUses;
    ...
    for (const [toolId, stats] of statsMap) {
      ...
      stats.effectivenessScore =
        stats.completionRate * 0.6 + frequencyScore * 0.2 + recencyBonus;

      // Calculate preferred times
      const timeUsage = toolUsage.reduce(
        (acc, u) => {
    ...
    localStorage.setItem(
      this.STATS_KEY,
      JSON.stringify(Array.from(statsMap.values())),
    );
  }
  ...
  static getRecommendations(context: RecommendationContext): string[] {
    ...
    const allTools = Object.keys(TOOL_CATEGORIES) as Array<
      keyof typeof TOOL_CATEGORIES
    >;
    ...
    for (const toolId of allTools) {
      ...
      if (toolStats && toolStats.totalUses > 0) {
        ...
        if (
          ["quick-access", "physiological-sigh", "5-4-3-2-1"].includes(toolId)
        ) {
          score += 20;
      ...
      if (context.timeAvailable) {
        ...
        if (context.timeAvailable === "quick" && toolDuration <= 180) {
          ...
          context.timeAvailable === "moderate" &&
          toolDuration <= 600
        ) {
          score += 20;
      ...
      if (
        ...
        (toolInfo.stress as readonly string[]).includes(context.recentStress)
      ) {
        score += 30;
      ...
      if (
        ...
        ["breathing-space", "present-moment"].includes(toolId)
      ) {
        score += 10;
      ...
      if (
        ...
        ["body-scan", "progressive-relaxation"].includes(toolId)
      ) {
        score += 10;
  ...
  static getInsight(): string {
    ...
    if (mostUsedTool && mostUsedTool.totalUses > 3) {
      insights.push(
        `${mostUsedTool.toolId.replace("-", " ")} seems to work well for you`,
      );
    }
    ...
    if (usage.length > 10) {
      const recentWeek = usage.filter(
        (u) => Date.now() - u.timestamp < 7 * 24 * 60 * 60 * 1000,
      );
    ...
    return insights.length > 0
      ? insights[0]
      : "Keep exploring to find your perfect techniques!";
  }
}
